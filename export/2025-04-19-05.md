...
通过对RESOLVE和ECO气体在星系组（G3）中的作用的研究，我们考察了星系组综合HI与晕质量比（$M_{\rm HI,grp}/M_{\rm halo}$）和星系组X射线发射的可能驱动因素，包括组晕质量（$M_{\rm halo}$）、通过穿越时间（$t_{\rm cross}$）探测的热化状态、活跃星系核（AGN）的存在以及组综合恒星质量增长率（FSMGR$_{\rm grp}$）。G3组的质量范围为$M_{\rm halo}=10^{11-14.5}\,M_\odot$，并结合了ROSAT全天数据的X射线堆叠数据。我们确认在$M_{\rm halo}=10^{12.6-14}\,M_\odot$以及$M_{\rm halo}>10^{14}\,M_\odot$的情况下检测到的热气体发射超出了AGN和X射线双星的背景，反映出$M_{\rm\,HI,grp}/M_{\rm halo}$与热气体发射的逆相关关系。固定晕质量下，当$t_{\rm cross} \sim 2$ Gyr时，$M_{\rm\,HI,grp}/M_{\rm halo}$将向更大的分布过渡。通过对这一过渡进行分组，$t_{\rm cross}$较低的组在$M_{\rm halo}>10^{13.3}\,M_\odot$时表现出较高的X射线发射，而这一趋势在$M_{\rm halo}=10^{12.6-13.3}\,M_\odot$时则相反。此外，$M_{\rm halo} \sim 10^{12.1}\,M_\odot$以下的AGN宿主晕相比于非AGN宿主晕表现出一个深约0.25 dex的$M_{\rm HI,grp}/M_{\rm halo}$的宽阔谷。随着非AGN宿主晕的混合，该谷变得更为平坦，约落在$M_{\rm halo}=10^{11.5}\,M_\odot$和$M_{\rm halo}=10^{12.1}\,M_\odot$之间的总体$M_{\rm\,HI,grp}/M_{\rm\,halo}$与$M_{\rm halo}$关系中。此外，我们可能检测到一个位于$M_{\rm halo} \sim 10^{13}\,M_\odot$的第二个不易解释的谷。两个谷都不符合理论预测，在$M_{\rm halo}=10^{12.1}\,M_\odot$及以上应该有更深的谷。

**Summary**:

- (1): 本文研究中性氢（H I）在星系组中的作用及其影响因素，旨在探讨H I内容与星系组环境之间的关系，以及该过程中活跃星系核（AGN）对H I内容的影响。

- (2): 过去的研究主要聚焦于H I与晕质量关系（$M_{\rm HI,grp}-M_{\rm halo}$），但无法充分考察物理驱动因素对这一关系的影响。本文通过分析多个因素（如晕质量、热化状态、AGN存在及恒星质量增长率）相互作用，将其与H I内容和X射线发射结合，展现了更全面的分析方法。

- (3): 本文的贡献在于通过系统性的方法研究H I与X射线发射之间的相互依赖性，发现H I内容与晕质量的逆相关关系，以及AGN对H I内容的显著影响，这为理解星系演化提供了新的见解。

- (4): 研究采用了综合HI与AGN信息、ROSAT全天数据的X射线堆叠分析等方法，结合星系组的晕质量和动态状态，从而探讨它们与H I和X射线发射之间的关系。

- (5): 本文的研究成果显示，在特定晕质量范围内，存在H I与X射线发射的显著联系，对AGNs的影响也表明了H I的丰富性与星系组的演化及环境因素相关。这些发现支持了论文的研究目标。
...
詹姆斯·韦布空间望远镜（JWST）揭示了一种在 z ≳ 5 的红色紧凑源群体，称为“小红点”（Little Red Dots，LRDs），这些对象可能是活动星系核（AGNs）。在这项研究中，我们对314个LRDs进行了综合性变异性研究，利用在五个深场（UDS、GOODS-S、GOODS-N、Abell 2744和COSMOS）中获得的多次JWST观测数据。我们使用在这些领域中公开可用的JWST NIRCam成像数据和多次JWST MIRI图像，对所有LRDs的变异性进行显著性测量，并通过信噪比（SNR_var）分布对其变异性进行统计评估。我们特别关注不同观测时期之间光度零点的系统偏差，这似乎是普遍存在的。LRDs的SNR_var分布，包括那些有宽Hα/Hβ发射线的LRDs，遵循标准高斯分布，并与在同一图像中检测到的比较样本相一致。这一发现表明LRD群体在平均上并未表现出强烈的变异性，这可以通过AGNs中超爱丁顿辐射的黑洞来解释。或者，它们中的许多可能以星系为主。我们还发现了八个强变LDR候选对象，其变异幅度在0.24到0.82等明亮度变化范围内。光谱能量分布（SED）表明这些可变LRDs应该有显著的AGN贡献。未来的JWST观测将提供更多LRDs的变异性信息。

**Summary**:

- (1): 文章的研究背景是通过JWST发现一类在红移 z ≳ 5 的紧凑红色源群体（LRDs），这些对象被认为是潜在的活动星系核（AGNs）。

- (2): 以往研究主要依赖静态光谱和光度数据，存在对LRDs的本质判断存在争议的问题。针对这一问题，本文提出通过变异性研究来确定LRDs的特性，这一方法可以更好地区分AGNs和星系，提供更有力的证据。

- (3): 本文的贡献在于对314个LRDs的变异性进行综合分析，并首次发现这些对象平均上表现出较弱的变异性，同时识别出八个具有较强变异性的新候选对象。

- (4): 论文采用的研究方法包括利用JWST的NIRCam和MIRI成像数据，通过计算信噪比（SNR_var）来评估LRDs的变异性，并对不同观测时期的系统偏差进行统计分析。

- (5): 本文的任务是对LRDs的变异性进行分析，所取得的结果是识别出8个强变候选对象，并确认整体群体表现出较弱的变异性，这些结果支持了AGNs与星系混合模型的假设。
...
我们研究了863个红移在4到14之间的星系的紫外（UV）连续谱斜率β，使用了来自多个JWST GTO、ERS和GO项目（包括JADES、CEERS和UNCOVER）的归档JWST/NIRSpec PRISM光谱。在这些星系中，我们发现了一个显著的星系，位于红移z = 9.25，命名为EBG-1，具有显著的蓝紫外斜率β = -2.99 ± 0.15，不同于其他显示红色连续谱或因大不确定性而模糊的蓝色连续谱的星系。我们确认β值在数据处理和紫外发射/吸收线遮蔽的拟合波长范围内几乎没有变化。这种极蓝的斜率β = -3.0排除了尘埃消光或活动星系核（AGN）活动的显著贡献。与恒星和星云发射模型比较后，我们发现仅靠恒星模型无法重现如此蓝的UV斜率，即使在非常年轻、金属贫乏或重元素丰度偏高的连绵恒星形成情况下，强星云连续谱也会使UV斜率变红，但需要高的电离光子逃逸率，f_esc^ion > 0.5，低星云连续谱。虽然Hα发射线未被探测，可能是由于光谱灵敏度的限制，但我们发现给定恒星形成率（3 M⊙ yr^-1）和恒星质量（10^8.0 M⊙）的情况，弱的[O III] λ4959,5007发射线是平均发射线强度的三分之一，进一步表明高的电离光子逃逸率，f_esc^ion ~ 0.7或更高。EBG-1将为高红移星系的恒星和星云连续谱发射提供重要见解，是反映再电离时代电离光子逃逸现场的一个典范。

**Summary**:

- (1): 本文研究了早期宇宙中星系的紫外光谱斜率β，重点关注其对宇宙再电离的影响，关注年轻恒星的电离光子逃逸情况。

- (2): 过去的方法主要为光度法，但面临发射线污染和不确定性等问题。本文使用高质量光谱数据，与现有方法不同，通过对863个星系的系统分析，提供了更准确的UV光谱斜率测量。

- (3): 本文的贡献在于识别出一个具有极低紫外斜率（β = -2.99 ± 0.15）的星系EBG-1，提供了对高红移星系电离光子逃逸和星云发射的关键见解。

- (4): 采用JWST/NIRSpec PRISM光谱数据分析，系统测量并比较多个星系的UV斜率，评估其电离光子逃逸率，并进行模型比较。

- (5): 本文所提出的方法成功识别了具有高电离光子逃逸率的星系EBG-1，为理解早期宇宙中星系对再电离的贡献提供了支持，能够达到研究目标。
